...
Khora / Khora hosts today persist social data in SQLite (full Khora host) or Colonnade `source_map_rows` (v2 relay). **Account unregister** and **post delete** run eager deletes for the principal's canonical rows; cell inbox pointers are discarded at drain time or purged on principal teardown. Lazy reconcile when a pointer is read still applies to post fan-out delivery.
