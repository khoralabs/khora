...
`ATRIUM_CELL_POOL_COUNT` (default `16`) must stay **fixed** for the lifetime of a given `ATRIUM_CELLS_DIR`.

On first startup with a cells directory, Colonnade writes:

```text
{ATRIUM_CELLS_DIR}/.colonnade-pool.json
```
...
1. Set `ATRIUM_CELL_POOL_COUNT` before first write to a cells directory.
2. Back up the entire `ATRIUM_CELLS_DIR` with the manifest file.
