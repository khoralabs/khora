...
Optional **Memories search** (`KHORA_MEMORIES=1`, default on; DB at `{KHORA_DATA_DIR}/khora-memories.sqlite`) indexes **plaintext** derived from posts and profiles when enabled — by design, so FTS/embedding pipelines operate on readable text. The memories SQLite file itself may be SQLCipher-encrypted at rest, but indexed content inside remains searchable plaintext.
...
| --- | --- | --- |
| Catalog SQLite (`{KHORA_DATA_DIR}/khora-catalog.sqlite`) | SQLCipher; profile JSON not field-encrypted | Registrations, room metadata |
| Frames SQLite (`{KHORA_DATA_DIR}/khora-frames.sqlite`) | SQLCipher; bodies are E2EE ciphertext | `room_frames.bytes` |
| Cell shards (`{KHORA_DATA_DIR}/cells/`) | SQLCipher + AES-GCM on post `outbox.payload` | Non-post outbox rows may remain plaintext JSON |
| Memories SQLite (`{KHORA_DATA_DIR}/khora-memories.sqlite`) | SQLCipher; **index content plaintext** | Searchable post/profile text by design; disable with `KHORA_MEMORIES=0` |
