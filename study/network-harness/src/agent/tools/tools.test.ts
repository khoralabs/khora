...
describe("harness memory tools", () => {
  ...
  beforeEach(() => {
    ...
    env = createEnv({
        ...
        search: async (params) => {
          ...
          return merged
            ...
            .map((item, index) => ({
              ...
              labels: [],
              graph: { kind: "node" as const, nodeId: `node-${index}` },
            }));
        },
        persistence: {
      ...
      } as unknown as HarnessToolkitEnv["memoriesClient"],
    });
  });

  async function toolHandler(name: "writeMemory" | "writeSkill" | "searchMemories") {
