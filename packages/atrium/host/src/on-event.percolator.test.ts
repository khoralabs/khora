...
import { authorSubscriptionSearch } from "@khoralabs/atrium-contracts";
...
function createMemoriesStub() {
  return {
    ...
    close: () => {},
  } as unknown as import("./memories/bootstrap.ts").AtriumMemoriesHost;
}
...
describe("percolator inbox reasons", () => {
  test("tagged post fan-out adds standing_query for matching subscription", async () => {
    ...
    const cluster = createSqliteColonnadeCluster({
      cellsDirectory: `/tmp/atrium-percolator-${crypto.randomUUID()}`,
  ...
  test("public subscription fan-out adds standing_query for matching topic query", async () => {
    ...
    const encryption = createTestEncryptionMaterial();
    const cluster = createSqliteColonnadeCluster({
      cellsDirectory: `/tmp/atrium-percolator-sub-${crypto.randomUUID()}`,
      mode: { kind: "pool", cellCount: 2 },
      useCellWorkers: false,
      encryption: {
        sqlCipherKey: encryption.sqlCipherKey,
  ...
  test("author-follow standing query matches subscription in author posts namespace", async () => {
    ...
    const encryption = createTestEncryptionMaterial();
    const cluster = createSqliteColonnadeCluster({
      cellsDirectory: `/tmp/atrium-percolator-author-${crypto.randomUUID()}`,
      mode: { kind: "pool", cellCount: 2 },
      useCellWorkers: false,
      encryption: {
        sqlCipherKey: encryption.sqlCipherKey,
  ...
  test("private post with matching query does not fan out to non-author", async () => {
    ...
    const encryption = createTestEncryptionMaterial();
    const cluster = createSqliteColonnadeCluster({
      cellsDirectory: `/tmp/atrium-percolator-private-${crypto.randomUUID()}`,
      mode: { kind: "pool", cellCount: 2 },
      useCellWorkers: false,
      encryption: {
        sqlCipherKey: encryption.sqlCipherKey,
    ...
    percolator.percolator.registerQuery({
      ...
      search: {
        content: {},
        options: { labels: { some: ["atrium_topic:secret"] } },
      },
    });
  ...
  test("network post fans out only to connected peer with matching query", async () => {
    ...
    const peerProfile: AtriumProfile = {
      id: "prof-peer",
      username: "peer",
      displayName: "Peer",
    };
    const strangerProfile: AtriumProfile = {
      id: "prof-stranger",
      username: "stranger",
    ...
    const encryption = createTestEncryptionMaterial();
    const cluster = createSqliteColonnadeCluster({
      cellsDirectory: `/tmp/atrium-percolator-network-${crypto.randomUUID()}`,
      mode: { kind: "pool", cellCount: 2 },
      useCellWorkers: false,
      encryption: {
        sqlCipherKey: encryption.sqlCipherKey,
    ...
    percolator.percolator.registerQuery({
      ...
      search: {
        content: {},
        options: { labels: { some: ["atrium_topic:network"] } },
      },
    });
    percolator.percolator.registerQuery({
  ...
  test("post without matching subscription does not fan out standing_query", async () => {
    ...
    const encryption = createTestEncryptionMaterial();
    const cluster = createSqliteColonnadeCluster({
      cellsDirectory: `/tmp/atrium-percolator-post-${crypto.randomUUID()}`,
      mode: { kind: "pool", cellCount: 2 },
      useCellWorkers: false,
      encryption: {
        sqlCipherKey: encryption.sqlCipherKey,
