...
/** Public error raised by {@link AtriumDidAuth} guards; carries an HTTP status hint. */
...
export type AtriumDidAuthOptions = {
...
export type CreateAtriumDidAuthOptions = Omit<AtriumDidAuthOptions, "nonceStore"> & {
  /**
...
export class AtriumDidAuth {
  ...
  constructor(opts: AtriumDidAuthOptions) {
...
/**
 * Factory wrapping {@link AtriumDidAuth} with sensible defaults: SQLite-backed nonce store and
 * the did:key Ed25519 strategy. Pass a custom `nonceStore` or `strategy` to override.
 */
export function createAtriumDidAuth(opts: CreateAtriumDidAuthOptions): AtriumDidAuth {
  const nonceStore =
    opts.nonceStore ??
    (opts.db !== undefined
      ? createSqliteNonceStore(opts.db)
      : (() => {
          throw new Error("createAtriumDidAuth: provide `db` or `nonceStore`");
        })());
  return new AtriumDidAuth({
    nonceStore,
