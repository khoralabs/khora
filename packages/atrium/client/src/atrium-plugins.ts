...
export type AtriumPluginContext = {
  readonly client: import("./atrium-client").AtriumClient;
...
export const ATRIUM_BUILTIN_PLUGIN_ID = {
  profileSync: "atrium.plugin.profile-sync",
  telemetry: "atrium.plugin.telemetry",
  inboxBuffer: "atrium.plugin.inbox-buffer",
} as const;
...
export function mergeLabeledAtriumPluginLayers(
  ...
  if (collision === "first-wins") {
    ...
    return order.map((id) => {
      ...
      if (inst === undefined) throw new Error(`atrium: missing install for id ${id}`);
      return inst;
    });
  }
  const installById = new Map<string, AtriumPluginInstaller>();
