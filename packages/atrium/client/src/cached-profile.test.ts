...
describe("resolveProfileSyncPath", () => {
  test("returns absolute path joined with dataDir", () => {
    const p = resolveProfileSyncPath({
      dataDir: "/tmp/atrium",
      plugins: { "atrium.plugin.profile-sync": { filePath: "profile.json" } },
    });
    expect(p).toBe("/tmp/atrium/profile.json");
  });

  test("absolute filePath wins over dataDir", () => {
    const p = resolveProfileSyncPath({
      dataDir: "/tmp/atrium",
      plugins: { "atrium.plugin.profile-sync": { filePath: "/abs/state.json" } },
    });
  ...
  test("returns undefined when plugin disabled", () => {
    expect(
      resolveProfileSyncPath({
        plugins: { "atrium.plugin.profile-sync": false },
      }),
    ).toBeUndefined();
  });
