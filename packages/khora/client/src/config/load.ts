...
export type LoadKhoraAppConfigOptions<TSchema extends z.ZodTypeAny> = {
  ...
  /**
   * Entry file path:
   *  - `undefined`: caller is expected to have pre-resolved with `resolveKhoraConfigPath`; pass
...
export type LoadedKhoraAppConfig<TSchema extends z.ZodTypeAny> = {
 ...
 */
export function loadKhoraAppConfig<TSchema extends z.ZodTypeAny>(
  opts: LoadKhoraAppConfigOptions<TSchema>,
): LoadedKhoraAppConfig<TSchema> {
  let fileMerged: Record<string, unknown> | undefined;
  let sourcePath: string | undefined;
  let extendsChain: string[] = [];
  if (typeof opts.filePath === "string") {
    const fileRead = readKhoraConfigFileWithExtends(opts.filePath, {
  ...
  const merged = mergeKhoraAppConfigLayers(allLayers);
  const result = opts.schema.safeParse(merged);
  if (!result.success) {
    throw new KhoraConfigError(result.error.issues, sourcePath);
  }
