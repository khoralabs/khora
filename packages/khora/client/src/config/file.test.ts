...
describe("readKhoraConfigFileWithExtends", () => {
  test("merges single file with no extends", () => {
    ...
    const out = readKhoraConfigFileWithExtends(root, { fs });
  ...
  test("single-string extends resolves relative to referencing file", () => {
    ...
    const out = readKhoraConfigFileWithExtends(child, { fs });
  ...
  test("cycle detection throws KhoraConfigError", () => {
    ...
    expect(() => readKhoraConfigFileWithExtends(a, { fs })).toThrow(KhoraConfigError);
  });

  test("ENOENT throws when explicit", () => {
    const fs = makeFs({});
    expect(() =>
      readKhoraConfigFileWithExtends(path.resolve("/missing.json"), {
        fs,
        explicit: true,
      }),
    ).toThrow(KhoraConfigError);
  });

  test("ENOENT returns undefined when not explicit", () => {
    const fs = makeFs({});
    const r = readKhoraConfigFileWithExtends(path.resolve("/missing.json"), {
      fs,
      explicit: false,
    });
  ...
  test("invalid JSON surfaces with file path", () => {
    ...
    try {
      readKhoraConfigFileWithExtends(p, { fs });
      expect(false).toBe(true);
    } catch (e) {
      expect(e).toBeInstanceOf(KhoraConfigError);
      expect((e as KhoraConfigError).sourcePath).toBe(p);
    }
  });
});
