...
import { ColonnadePublicationClient } from "../../core/colonnade-publication-client";
...
import {
  ...
  resolveSourcemap,
} from "../../core/resolve-pointer";
import { createOutboxPayloadCodec } from "../../crypto";
...
function testCell(cellId: string): InMemoryCellPersistence {
  return new InMemoryCellPersistence(cellId, { outboxPayloadCodec: TEST_CODEC });
}
...
describe("Outbox field encryption", () => {
  ...
  test("encrypts post outbox payload and hashes ciphertext", async () => {
    const cell = new CellPersistenceClient(
      new InMemoryCellPersistence("cell-a", { outboxPayloadCodec: codec }),
    );
...
describe("InMemoryCellPersistence", () => {
