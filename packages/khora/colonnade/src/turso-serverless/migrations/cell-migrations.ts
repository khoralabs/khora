import { CELL_BASE_TABLES_DDL, SCHEMA_VERSION_TABLE_DDL, TURSO_PRAGMAS_DDL } from "../../core";
...
export async function migrateCellTursoServerless(db: TursoClients): Promise<void> {
  const applied = new Set(await listAppliedSchemaVersions(db));
  if (applied.has(COLONNADE_CELL_SCHEMA_VERSION)) {
    return;
  }
  const stmts = [
  ...
  await execMultiple(
    db.write,
    `INSERT INTO _schema_version (version, applied_at) VALUES ('${COLONNADE_CELL_SCHEMA_VERSION}', ${now});`,
  );
}
