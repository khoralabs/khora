...
export class ColonnadePublicationClient {
  ...
  private readonly resolveCell: ResolveCell;
  ...
  constructor(
    catalogOrResolveCell: CatalogPersistence | ResolveCell,
    maybeResolveCell?: ResolveCell,
  ) {
    if (maybeResolveCell !== undefined) {
      this.catalog = catalogOrResolveCell as CatalogPersistence;
      this.resolveCell = maybeResolveCell;
    } else if (isResolveCell(catalogOrResolveCell)) {
      this.catalog = defaultNoopCatalogPersistence();
      this.resolveCell = catalogOrResolveCell;
    } else {
      throw new Error(
        "ColonnadePublicationClient: pass ResolveCell alone, or (CatalogPersistence, ResolveCell)",
      );
    }
  }
