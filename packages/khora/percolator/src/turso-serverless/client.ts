...
/** Subset of Turso connection surface used by percolator adapters. */
...
export function createTursoClients(options: CreateTursoClientsOptions): TursoClients {
  ...
  return {
    config,
    read: options.read ?? (connect(config) as unknown as Connection),
    write: options.write ?? (connect(config) as unknown as Connection),
