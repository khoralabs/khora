...
function parseTrustedProxies(): string[] {
  const raw = process.env.TRUSTED_PROXIES?.trim() ?? process.env.REGISTRY_TRUSTED_PROXIES?.trim();
 ...
 * TRUSTED_PROXIES / REGISTRY_TRUSTED_PROXIES; otherwise use peer IP or "direct".
 *
