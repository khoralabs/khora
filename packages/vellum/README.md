...
Shared libraries for **Vellum**: NBC session tooling layered on Khora rooms — contracts for control/domain wires, an HTTP-oriented client, transport primitives for daemon control, and JSON-schema validation for bind policies.
...
| [`client/`](client) | `@khoralabs/vellum-client` | Programmatic API: talks to the Vellum daemon’s control HTTP surface and reads local SQLite state. Uses `@khoralabs/khora-client` for room/sign-in semantics where needed. |
...
khora-client     ◄──  vellum-client (room ticket / HTTP to Khora host)
```
