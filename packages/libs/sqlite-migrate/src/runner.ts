...
function prepareRun(db: Database, tableName: string, migrations: readonly Migration[]): RunPlan {
  ...
  const finalVersion = highestTo(ordered);
  return {
    pending,
    applied,
    result: { applied, skipped, finalVersion },
  };
}
...
function finalize(db: Database, plan: RunPlan): void {
  if (plan.result.finalVersion !== null) {
    db.run(`PRAGMA user_version = ${encodeSemverForUserVersion(plan.result.finalVersion)}`);
  }
}
...
function highestTo(migrations: readonly Migration[]): string | null {
