/* eslint-disable */
/**
 * Generated `ComponentApi` utility.
 *
 * THIS CODE IS AUTOMATICALLY GENERATED.
 *
 * To regenerate, run `npx convex dev`.
 * @module
 */

import type { FunctionReference } from "convex/server";

/**
 * A utility for referencing a Convex component's exposed API.
 *
 * Useful when expecting a parameter like `components.myComponent`.
 * Usage:
 * ```ts
 * async function myFunction(ctx: QueryCtx, component: ComponentApi) {
 *   return ctx.runQuery(component.someFile.someQuery, { ...args });
 * }
 * ```
 */
export type ComponentApi<Name extends string | undefined = string | undefined> =
  {
    actions: {
      searchVectorSourceMapIds: FunctionReference<
        "action",
        "internal",
        {
          limit: number;
          maxVectorDistance?: number;
          memoryIds?: Array<string>;
          scope: {
            kind: "unscoped" | "pathSubtree" | "scopeDag" | "exactScope";
            namespaces?: Array<string>;
            roots?: Array<string>;
            scopes?: Array<string>;
          };
          vector: Array<number>;
        },
        Array<string>,
        Name
      >;
    };
    mutations: {
      appendProvenanceEvent: FunctionReference<
        "mutation",
        "internal",
        { event: any; now: number },
        null,
        Name
      >;
      clearMemorySubtree: FunctionReference<
        "mutation",
        "internal",
        {
          edgeId?: string;
          memoryId: string;
          memoryKind: "node" | "edge";
          nodeId?: string;
        },
        null,
        Name
      >;
      deleteMemoryRootRows: FunctionReference<
        "mutation",
        "internal",
        {
          edgeId?: string;
          memoryId?: string;
          memoryKind: "node" | "edge";
          nodeId?: string;
        },
        null,
        Name
      >;
      ensureEdgeLabel: FunctionReference<
        "mutation",
        "internal",
        {
          description?: string;
          kind: string;
          now: number;
          schemaJson?: string | null;
        },
        string,
        Name
      >;
      ensureNodeLabel: FunctionReference<
        "mutation",
        "internal",
        {
          description?: string;
          kind: string;
          now: number;
          schemaJson?: string | null;
        },
        string,
        Name
      >;
      insertEdge: FunctionReference<
        "mutation",
        "internal",
        {
          fromNodeId: string;
          idParts: { fromMemoryId: string; label: string; toMemoryId: string };
          now: number;
          properties?: Record<string, any>;
          toNodeId: string;
        },
        { edgeId: string },
        Name
      >;
      insertEdgeLabelAssignment: FunctionReference<
        "mutation",
        "internal",
        {
          edgeId: string;
          labelId: string;
          now: number;
          props: Record<string, any>;
        },
        null,
        Name
      >;
      insertLexicalFeature: FunctionReference<
        "mutation",
        "internal",
        { memoryId: string; now: number; sourceMapId: string; text: string },
        { textFeatureId: string },
        Name
      >;
      insertNodeLabelAssignment: FunctionReference<
        "mutation",
        "internal",
        {
          labelId: string;
          nodeId: string;
          now: number;
          props: Record<string, any>;
        },
        null,
        Name
      >;
      insertSourceMap: FunctionReference<
        "mutation",
        "internal",
        { memoryId: string; now: number; sourceKey: string },
        { sourceMapId: string },
        Name
      >;
      insertVectorFeature: FunctionReference<
        "mutation",
        "internal",
        {
          memoryId: string;
          now: number;
          sourceMapId: string;
          vector: Array<number>;
        },
        { vectorFeatureId: string },
        Name
      >;
      linkScopes: FunctionReference<
        "mutation",
        "internal",
        { childScopeId: string; now: number; parentScopeId: string },
        null,
        Name
      >;
      mergeMemoryAtomic: FunctionReference<
        "mutation",
        "internal",
        {
          attachScopes?: Array<string>;
          content: Array<{
            key: string;
            text?: string;
            vector?: Array<number>;
          }>;
          edge?: {
            from_memory_id: string;
            label: { kind: string; props: Record<string, any> };
            properties?: Record<string, any>;
            to_memory_id: string;
          };
          edges?: Array<{
            direction: "in" | "out";
            label: { kind: string; props: Record<string, any> };
            peer_memory_id: string;
            properties?: Record<string, any>;
          }>;
          key: string;
          kind?: "node" | "edge";
          labels?: Array<{ kind: string; props: Record<string, any> }>;
          namespace: string;
          now: number;
          properties?: Record<string, any>;
          searchMetaVector?: Array<number>;
        },
        Array<string>,
        Name
      >;
      replaceMemoryScopes: FunctionReference<
        "mutation",
        "internal",
        { memoryId: string; now: number; scopeIds: Array<string> },
        null,
        Name
      >;
      syncLabelPropsSearchFeatures: FunctionReference<
        "mutation",
        "internal",
        { memoryKey: string; namespace: string; now: number },
        null,
        Name
      >;
      syncMemorySearchMeta: FunctionReference<
        "mutation",
        "internal",
        {
          memoryKey: string;
          metaVector?: Array<number>;
          namespace: string;
          now: number;
        },
        null,
        Name
      >;
      unlinkScopeEdge: FunctionReference<
        "mutation",
        "internal",
        { childScopeId: string; now: number; parentScopeId: string },
        null,
        Name
      >;
      updateSourceMapContentHash: FunctionReference<
        "mutation",
        "internal",
        { contentHash: string; sourceMapId: string },
        null,
        Name
      >;
      upsertMemory: FunctionReference<
        "mutation",
        "internal",
        {
          edgeId?: string | null;
          key: string;
          kind?: "node" | "edge";
          namespace: string;
          now: number;
        },
        { _ts_created: number; memoryId: string },
        Name
      >;
      upsertMemorySearchMetaVector: FunctionReference<
        "mutation",
        "internal",
        {
          memoryKey: string;
          namespace: string;
          now: number;
          vector: Array<number>;
        },
        null,
        Name
      >;
      upsertNodeForMemoryKey: FunctionReference<
        "mutation",
        "internal",
        {
          memoryId: string;
          memoryKey: string;
          namespace: string;
          now: number;
          properties?: Record<string, any>;
        },
        { nodeId: string },
        Name
      >;
      upsertScope: FunctionReference<
        "mutation",
        "internal",
        { now: number; scopeId: string },
        null,
        Name
      >;
    };
    queries: {
      buildCanonicalMemorySearchMetaTextQuery: FunctionReference<
        "query",
        "internal",
        { memoryKey: string; namespace: string },
        string,
        Name
      >;
      findMemoryAssociation: FunctionReference<
        "query",
        "internal",
        { key: string; namespace: string },
        | null
        | { kind: "node"; memoryId: string; nodeId: string }
        | { edgeId: string; kind: "edge"; memoryId: string },
        Name
      >;
      findMemoryIdByKey: FunctionReference<
        "query",
        "internal",
        { key: string; namespace: string },
        string | null,
        Name
      >;
      getLexicalTextForMemorySource: FunctionReference<
        "query",
        "internal",
        { memoryId: string; sourceKey: string },
        string | null,
        Name
      >;
      getProvenanceHeadRootHex: FunctionReference<
        "query",
        "internal",
        {},
        string | null,
        Name
      >;
      hydrateSourceMapHits: FunctionReference<
        "query",
        "internal",
        { sourceMapIds: Array<string> },
        Array<{
          _id: string;
          _ts_created: number;
          graph:
            | { kind: "node" }
            | {
                edge: {
                  directed?: boolean;
                  edgeId: string;
                  fromKey: string;
                  labels: Array<{ kind: string; props: Record<string, any> }>;
                  properties?: Record<string, any> | null;
                  toKey: string;
                };
                kind: "edge";
              };
          labels: Array<{ kind: string; props: Record<string, any> }>;
          memory: {
            _id: string;
            _ts_created: number;
            edge_id?: string;
            key: string;
            kind?: "node" | "edge";
            namespace: string;
          };
          memory_id: string;
          source_key: string;
        }>,
        Name
      >;
      listIncidentGraphEdges: FunctionReference<
        "query",
        "internal",
        { memoryKey: string; namespace: string },
        Array<{
          directed?: boolean;
          edgeId: string;
          fromKey: string;
          labels: Array<{ kind: string; props: Record<string, any> }>;
          properties?: Record<string, any> | null;
          toKey: string;
        }>,
        Name
      >;
      listMemoriesInNamespace: FunctionReference<
        "query",
        "internal",
        { namespace: string },
        Array<{
          bodyText: string | null;
          key: string;
          memoryId: string;
          tsCreated: number;
        }>,
        Name
      >;
      listNeighborMemoriesForNodeQuery: FunctionReference<
        "query",
        "internal",
        { namespace: string; nodeId: string },
        Array<{ key: string; namespace: string }>,
        Name
      >;
      listNeighborsForEdgeMemory: FunctionReference<
        "query",
        "internal",
        {
          edgeId: string;
          filters?: {
            all?: Array<{
              direction?: "in" | "out";
              label: string;
              nodes?: { all?: Array<string>; some?: Array<string> };
            }>;
            some?: Array<{
              direction?: "in" | "out";
              label: string;
              nodes?: { all?: Array<string>; some?: Array<string> };
            }>;
          };
          namespace: string;
        },
        Array<{
          _id: string;
          _ts_created: number;
          edge: {
            _id: string;
            _ts_created: number;
            from_node_id: string;
            label: { kind: string; props: Record<string, any> };
            properties?: Record<string, any>;
            to_node_id: string;
          };
          key: string;
          labels: Array<{ kind: string; props: Record<string, any> }>;
          namespace: string;
        }>,
        Name
      >;
      listNeighborsForMemory: FunctionReference<
        "query",
        "internal",
        {
          filters?: {
            all?: Array<{
              direction?: "in" | "out";
              label: string;
              nodes?: { all?: Array<string>; some?: Array<string> };
            }>;
            some?: Array<{
              direction?: "in" | "out";
              label: string;
              nodes?: { all?: Array<string>; some?: Array<string> };
            }>;
          };
          key: string;
          namespace: string;
        },
        Array<{
          _id: string;
          _ts_created: number;
          edge: {
            _id: string;
            _ts_created: number;
            from_node_id: string;
            label: { kind: string; props: Record<string, any> };
            properties?: Record<string, any>;
            to_node_id: string;
          };
          key: string;
          labels: Array<{ kind: string; props: Record<string, any> }>;
          namespace: string;
        }>,
        Name
      >;
      listScopesForMemory: FunctionReference<
        "query",
        "internal",
        { memoryId: string },
        Array<string>,
        Name
      >;
      listSourceMapsForMemory: FunctionReference<
        "query",
        "internal",
        { limit: number; memoryId: string },
        Array<{
          _id: string;
          _ts_created: number;
          content_hash?: string;
          memory_id: string;
          source_key: string;
        }>,
        Name
      >;
      listTextFeatureExportRowsForMemory: FunctionReference<
        "query",
        "internal",
        { memoryId: string },
        Array<{ memory_id: string; source_key: string; text: string }>,
        Name
      >;
      listVectorEmbeddingIndexDimensions: FunctionReference<
        "query",
        "internal",
        {},
        Array<number>,
        Name
      >;
      loadGraphEdge: FunctionReference<
        "query",
        "internal",
        { edgeId: string; namespace: string },
        {
          directed?: boolean;
          edgeId: string;
          fromKey: string;
          labels: Array<{ kind: string; props: Record<string, any> }>;
          properties?: Record<string, any> | null;
          toKey: string;
        } | null,
        Name
      >;
      loadGraphEdgesForNamespace: FunctionReference<
        "query",
        "internal",
        { namespace: string },
        Array<{
          directed?: boolean;
          edgeId: string;
          fromKey: string;
          labels: Array<{ kind: string; props: Record<string, any> }>;
          properties?: Record<string, any> | null;
          toKey: string;
        }>,
        Name
      >;
      loadGraphNode: FunctionReference<
        "query",
        "internal",
        { memoryKey: string; namespace: string },
        {
          labels: Array<{ kind: string; props: Record<string, any> }>;
          memoryKey: string;
          namespace: string;
          nodeId: string;
          properties: Record<string, any> | null;
        } | null,
        Name
      >;
      loadMemoryNamespaceKey: FunctionReference<
        "query",
        "internal",
        { memoryId: string },
        null | { key: string; namespace: string },
        Name
      >;
      loadNodeLabelsForMemory: FunctionReference<
        "query",
        "internal",
        { memoryKey: string; namespace: string },
        Array<{ kind: string; props: Record<string, any> }>,
        Name
      >;
      loadNodeLabelsForNamespace: FunctionReference<
        "query",
        "internal",
        { namespace: string },
        Array<{
          labels: Array<{ kind: string; props: Record<string, any> }>;
          memoryKey: string;
        }>,
        Name
      >;
      loadNodePropertiesForMemory: FunctionReference<
        "query",
        "internal",
        { memoryKey: string; namespace: string },
        Record<string, any> | null,
        Name
      >;
      loadNodePropertiesForNamespace: FunctionReference<
        "query",
        "internal",
        { namespace: string },
        Array<{ memoryKey: string; properties: Record<string, any> | null }>,
        Name
      >;
      nodeExists: FunctionReference<
        "query",
        "internal",
        { nodeId: string },
        boolean,
        Name
      >;
      searchLexicalSourceMapIds: FunctionReference<
        "query",
        "internal",
        {
          limit: number;
          memoryIds?: Array<string>;
          scope: {
            kind: "unscoped" | "pathSubtree" | "scopeDag" | "exactScope";
            namespaces?: Array<string>;
            roots?: Array<string>;
            scopes?: Array<string>;
          };
          text: string;
        },
        Array<string>,
        Name
      >;
    };
  };
