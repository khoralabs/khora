...
export class TursoCellPersistenceStrategy implements CellPersistenceStrategy {
  ...
  ): Promise<TursoCellPersistenceStrategy> {
    if (opts.autoMigrate !== false) {
      await migrateCellTursoServerless(db);
    }
    return new TursoCellPersistenceStrategy(db, cellId, opts.outboxPayloadCodec);
  }
  ...
  async ensureCellPoolCount(expected: number): Promise<void> {
    ...
    if (stored !== expected) {
      throw new Error(
        `TursoCellPersistenceStrategy: cell_pool_count mismatch (stored ${stored}, expected ${expected})`,
      );
    }
  }
  ...
  private assertCell(cell_id: string): void {
    if (cell_id !== this.cellId) {
      throw new Error(`TursoCellPersistenceStrategy: cell_id mismatch (expected ${this.cellId})`);
    }
  }
  ...
  async fetchOutboxPayload(input: FetchOutboxPayloadInput): Promise<FetchOutboxPayloadOutput> {
    ...
    if (input.locator.cell_id !== this.cellId) {
      throw new Error("TursoCellPersistenceStrategy: locator.cell_id mismatch");
    }
