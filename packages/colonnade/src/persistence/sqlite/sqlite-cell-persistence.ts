...
import type { CellBatchCapable, CellPersistence, DiscardInboxEntriesInput } from "../core";
import {
...
export class SqliteCellPersistence implements CellPersistence {
  ...
  private assertCell(cell_id: string): void {
    if (cell_id !== this.cellId) {
      throw new Error(`SqliteCellPersistence: cell_id mismatch (expected ${this.cellId})`);
    }
  }
  ...
  fetchOutboxPayload(input: FetchOutboxPayloadInput): Promise<FetchOutboxPayloadOutput> {
    ...
    if (input.locator.cell_id !== this.cellId) {
      throw new Error("SqliteCellPersistence: locator.cell_id mismatch");
    }
