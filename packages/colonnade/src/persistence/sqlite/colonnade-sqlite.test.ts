...
import { createSqliteColonnadeCluster, SqliteCatalogPersistence } from "./index";
...
describe("SQLite Colonnade cluster", () => {
  ...
  test("catalog shards: tenant-key routing stores pointers on the correct SQLite file", async () => {
    ...
    const shardDbs = shardPaths.map((p) => openCatalogDb(p));
    const catalog = new ShardingCatalogPersistence(
      shardDbs.map((db, i) => new SqliteCatalogPersistence(db, { shardIndex: i })),
    );
    const cluster = createSqliteColonnadeCluster({
      catalog,
  ...
  test(
    "useCellWorkers: publication + inbox pointer staging round-trip",
    async () => {
      ...
      try {
        ...
        const catalogDb = openMaybeEncryptedDatabaseSync(
          join(dir, "catalog.sqlite"),
          { create: true },
          testEncryption.sqlCipherKey,
        );
        const catalog = new SqliteCatalogPersistence(catalogDb);
        const cluster = createSqliteColonnadeCluster({
          catalog,
