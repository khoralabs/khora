...
import { parseCatalogPointerShardIndex } from "../../core/routing/catalog-pointer-id";
import { catalogShardIndexForTenant } from "../../core/routing/tenant-catalog-shard";
...
export class ShardingCatalogPersistence implements CatalogPersistence {
  ...
  constructor(private readonly shards: readonly CatalogPersistence[]) {
    if (shards.length < 1) {
      throw new Error("ShardingCatalogPersistence: need at least one shard");
    }
    this.nextCatalogPointerId = (tenantKey: string) => {
      const i = catalogShardIndexForTenant(tenantKey, shards.length);
      const leaf = shards[i];
      if (leaf === undefined) {
        throw new Error("ShardingCatalogPersistence: shard index out of range");
      }
      const gen = leaf.nextCatalogPointerId;
      if (gen === undefined) {
        throw new Error("ShardingCatalogPersistence: shard missing nextCatalogPointerId");
      }
  ...
  private shardForTenantKey(tenantKey: string): CatalogPersistence {
  ...
  private shardForDiscoveryKey(documentKey: string): CatalogPersistence {
