...
import { assertContentHash, randomId, sha256HexLower } from "../../core/hash";
 ...
 */
export class InMemoryCellPersistence implements CellPersistence {
  ...
  private assertCell(cell_id: CellId): void {
    if (cell_id !== this.cellId) {
      throw new Error(`InMemoryCellPersistence: cell_id mismatch (expected ${this.cellId})`);
    }
  }
  ...
  async fetchOutboxPayload(input: FetchOutboxPayloadInput): Promise<FetchOutboxPayloadOutput> {
    ...
    if (input.locator.cell_id !== this.cellId) {
      throw new Error("InMemoryCellPersistence: locator.cell_id mismatch");
    }
...
function cloneWriteOp(
  op: import("../../core/colonnade-types.ts").WriteOp,
): import("../../core/colonnade-types.ts").WriteOp {
...
function verifyStaging(
  ...
  resolved: import("../../core/colonnade-types.ts").ResolvedPayload | undefined,
): boolean {
