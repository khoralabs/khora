...
export interface ObpPersistence {
  ...
  /** Set port **`expires_seq`** (revocation / soft-close at ledger sequence). Caller enforces ownership policy. */
  setPortExpiresSeq(portId: string, expiresSeq: number): void;

  /**
   * Set offer **`expires_seq`** and cascade the same sequence to all ports exposed on that offer.
