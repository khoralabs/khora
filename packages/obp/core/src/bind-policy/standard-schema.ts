...
const CHOICE_CONSTRAINT_KEYS = new Set(["choices", "maxSelections"]);
...
function validateChoiceLeaf(
  ...
  if (value.length < 1) {
    out.push(issue("Array must contain at least 1 element(s)", path));
  }
...
function validateCounterpartyBindValue(
...
/**
 * Standard Schema validator for `counterparty_bind` payloads keyed by slugs derived from
 ...
 */
export function counterpartyBindSchemaForProperties(
  properties: readonly BindPolicyField[],
  ...
  return {
    "~standard": {
      version: 1,
      vendor: VENDOR,
      validate: (value: unknown) => validateCounterpartyBindValue(properties, value),
      types: {
        input: undefined as unknown,
