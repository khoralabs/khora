...
describe("partyIdForSigner", () => {
  test("returns id for signer pubkey", () => {
    const init: SessionInitNormalized = { session_id: "s", genesis_hash: gh, parties: [pB, pA] };
    expect(partyIdForSigner(init, "0x01")).toBe("party-b");
  });

  test("unknown signer throws", () => {
    const init: SessionInitNormalized = { session_id: "s", genesis_hash: gh, parties: [pA, pB] };
