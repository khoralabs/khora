...
/// **Normative rules (NBC)**
///
/// **N1. Expiry at bind time.** Reject **BindPort** / bind leg of **ExtendOffer** when the binding **offer** or target **port** NBC bind window is expired per **either** active mode: resolve **`expires_turn`** / **`expires_at_relay_ms`** from **`NbcTurnBody`** / persisted **`nbc_expires_*`** projection for those rows (not from thin **`khora.obp#Offer` / `Port`**). If **`expires_turn ≠ 0`**, require **`turn_seq < expires_turn`**; if **`expires_at_relay_ms ≠ 0`**, require **`relay_ts_ms < expires_at_relay_ms`** where **`relay_ts_ms`** is taken from **`khora.agent.relay#RelayEnvelope`** on the **TURN** when hub relay policy applies (same envelope for both parties). **`turn_seq`** is the count of DAG-committed frames on the chain before applying that **TURN** (implementation-defined equivalence to session ops).
///
