...
/// **Normative rules (NBC)**
///
/// **N1. Expiry at bind time.** Reject **BindPort** / bind leg of **ExtendOffer** when the binding **offer** or target **port** NBC bind window is expired per **either** active mode: resolve **`expires_turn`** / **`expires_at_relay_ms`** from **`NbcTurnBody`** / persisted **`nbc_expires_*`** projection for those rows (not from thin **`cfd.obp#Offer` / `Port`**). If **`expires_turn ≠ 0`**, require **`turn_seq < expires_turn`**; if **`expires_at_relay_ms ≠ 0`**, require **`relay_ts_ms < expires_at_relay_ms`** where **`relay_ts_ms`** is taken from **`cfd.agent.relay#RelayEnvelope`** on the **TURN** when hub relay policy applies (same envelope for both parties). **`turn_seq`** is the count of DAG-committed frames on the chain before applying that **TURN** (implementation-defined equivalence to session ops).
///
...
///
/// **N4. Bind policy.** When an expose path carries non-empty **`NbcPortExposePolicy.bind_policy`**, **BindPort** / bind leg of **ExtendOffer** MUST supply **`bind_payload`** (`Document` on the **`ObpPersistence`** operation / listing surface) whose bytes validate against that policy before committing the **BINDS** edge; validated payload is stored with the bind row. **`bind_policy_snapshot`** on listing rows MAY hold an **`NbcBindPolicyAuditSnapshot`**. Structured JSON for **`bind_policy`** is **product/host-defined**; NBC requires binding implementations run **that deployment's** validator at bind time (reference stacks MAY ship a concrete schema profile).
///
...
///
/// **N9. Row `created_seq` (commit metadata).** Implementations **MAY** persist a monotonic **`created_seq`** per stored graph row using **`cfd.obp.nbc#NbcRowCommitMeta`** semantics. It is **not** part of **`cfd.obp`** Smithy shapes (`packages/obp/v2/model/spec/model/shapes.smithy`); it supports NBC / adapter ordering and audit.
///
