...
/// while each runs **`cfd.obp#ObpPersistence`** locally. **`cfd.obp`** **`shapes.smithy`** / **`persistence.smithy`** MUST NOT
...
///
/// **Ledger alignment:** Bind/expose validity continues to use **`ledger_seq < expires_seq`** on **`Offer`** / **`Port`**
/// from **`cfd.obp`** (see **`persistence.smithy`**). **`ledger_seq`** is advanced by the session driver on commit—not wall clock.
///
