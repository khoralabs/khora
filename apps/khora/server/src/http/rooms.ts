...
function relationshipRowToApiItem(
  ...
  return zKhoraRelationshipItem.parse({
...
export async function handleRoomsCreate(
  ...
  let body: z.infer<typeof zKhoraRoomCreateBody>;
  try {
    body = zKhoraRoomCreateBody.parse(JSON.parse(bodyText) as unknown);
  } catch (e) {
  ...
  if (targetDidResolved === undefined) {
    ...
    return Response.json(zKhoraRoomCreateResponse.parse({ ...ticketPayload, joinToken }));
  }
  return Response.json(zKhoraRoomCreateResponse.parse(ticketPayload));
}
...
export async function handleRoomsJoin(
  ...
  let joinBody: z.infer<typeof zKhoraRoomJoinRequestBody>;
  try {
    joinBody = zKhoraRoomJoinRequestBody.parse(JSON.parse(bodyText) as unknown);
  } catch (e) {
    const msg = e instanceof z.ZodError ? e.message : "Invalid JSON body";
    return jsonError(msg, 400);
  }
...
export async function handleRoomsMintTicket(
  ...
  let mintBody: z.infer<typeof zKhoraRoomMintTicketBody> = {};
  if (bodyText.trim().length > 0) {
    try {
      mintBody = zKhoraRoomMintTicketBody.parse(JSON.parse(bodyText) as unknown);
    } catch (e) {
      const msg = e instanceof z.ZodError ? e.message : "Invalid JSON body";
      return jsonError(msg, 400);
    }
