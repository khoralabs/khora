...
type ShareSessionContentProps = {
  sessionId: string;
  active?: boolean;
  onChanged?: () => void;
};

export function ShareSessionContent({
  sessionId,
  active = true,
  onChanged,
}: ShareSessionContentProps) {
              ...
              <Popover open={addOpen} onOpenChange={setAddOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {candidates.length === 0 ? "Everyone is already added" : "Add people or teams…"}
                  </Button>
                </PopoverTrigger>
                    ...
                    <CommandList>
                      ...
                      {candidates.some((c) => c.kind === "team") ? (
                        <CommandGroup heading="Teams">
                          {candidates
                            .filter(
                              (c): c is Extract<Candidate, { kind: "team" }> => c.kind === "team",
                            )
                            .map((c) => (
                              <CommandItem
                                key={c.team.team.id}
