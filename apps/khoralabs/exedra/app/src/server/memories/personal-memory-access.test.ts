...
test("grantPersonalMemoryAccessForSession grants org reader and records consent", async () => {
  ...
  const teamId = await createTeam(db, { orgId, name: "TeamPM", ownerId: orgOwner.id });
  ...
  expect(await canReadPersonalKg(orgId, participant.id)).toBe(true);
  expect(hasPersonalMemoryConsent(db, session.id, participant.id)).toBe(true);
  expect(
    await canOrgAgentAccessParticipantPersonalMemories(db, {
      orgId,
      sessionId: session.id,
...
test("releasePersonalMemoryAccessForSession revokes grant when no other consented sessions", async () => {
  ...
  const teamId = await createTeam(db, { orgId, name: "TeamPM2", ownerId: orgOwner.id });
  ...
  await releasePersonalMemoryAccessForSession(db, session.id);

  expect(await canReadPersonalKg(orgId, participant.id)).toBe(false);
...
test("releasePersonalMemoryAccessForSession keeps grant when another session has consent", async () => {
  ...
  const teamId = await createTeam(db, { orgId, name: "TeamPM3", ownerId: orgOwner.id });
  const sessionA = createSession(db, { teamId, topic: "PM session A" });
  const sessionB = createSession(db, { teamId, topic: "PM session B" });
  await grantSessionParticipant(participant.id, sessionA.id);
  await grantSessionParticipant(participant.id, sessionB.id);
  ...
  await releasePersonalMemoryAccessForSession(db, sessionA.id);
...
test("canOrgAgentAccessParticipantPersonalMemories denies without grant", async () => {
  ...
  const teamId = await createTeam(db, { orgId, name: "TeamPM4", ownerId: orgOwner.id });
  ...
  await revokePersonalKgReader(orgId, participant.id);
});
