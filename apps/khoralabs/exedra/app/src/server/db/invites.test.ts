...
test("session invite public info includes org details", async () => {
  ...
  const teamId = await createTeam(db, { orgId, name: "Leadership", ownerId: user.id });
...
test("team invite is single-use and grants membership on accept", async () => {
  ...
  const teamId = await createTeam(db, { orgId, name: "TeamInvite", ownerId: owner.id });
  ...
  if (effects !== null) await applyInviteEffects(db, joiner.id, effects);
...
test("user with session access is treated as already joined", async () => {
  ...
  expect(effects).not.toBeNull();
  if (effects !== null) await applyInviteEffects(db, participant.id, effects);
  await grantSessionParticipant(participant.id, session.id);

  expect(await userHasSessionAccess(db, session.id, participant.id)).toBe(true);
