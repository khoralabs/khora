...
describe("turn relay", () => {
  test("forwards events to registered listeners", () => {
    ...
    relayTurnEvent(threadId, { type: "text_delta", delta: "hi" });
    expect(received).toHaveLength(1);
    expect(received[0]).toEqual({ type: "text_delta", delta: "hi" });

    cleanup();
    relayTurnEvent(threadId, { type: "text_delta", delta: "ignored" });
    expect(received).toHaveLength(1);
  });
});
