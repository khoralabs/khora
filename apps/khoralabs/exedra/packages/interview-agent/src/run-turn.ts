...
async function runInterviewTurnSession(args: {
  ...
  const env: InterviewEnv = {
    ...
    ...(memorySearch !== undefined
      ? {
          searchOrgMemories: memorySearch.searchOrgMemories,
          ...(memorySearch.searchPersonalMemories !== undefined
            ? { searchPersonalMemories: memorySearch.searchPersonalMemories }
            : {}),
        }
      : {}),
  };
...
export async function runInterviewTurn(args: {
  ...
  const tel = (
    ...
    (() => {
      throw new Error("runInterviewTurn requires createTelemetry");
    })
  )();
