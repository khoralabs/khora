...
async function mergeToBothPartyNamespaces(
  runId: string,
  model: LanguageModel,
  makePayload: (namespace: string) => { payload: MeetingSeedPayload; memoryKey: string; correlationId: string },
): Promise<void> {
  ...
  const bundle = createMatchmakingMemoriesBundle(resolveMemoriesDbPath(root), { memoriesRoot: root });
...
export async function mergePostNegotiationReviewToPartyKgs(args: {
  ...
  const payload: MeetingSeedPayload = {
    ...
    decision,
    ...(agentFeedback !== undefined && agentFeedback.length > 0
      ? { agentFeedback }
      : {}),
  };
