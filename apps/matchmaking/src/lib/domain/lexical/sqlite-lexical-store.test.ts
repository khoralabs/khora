...
test("SqliteLexicalStore resolve after syncFromTextExportRows (last write wins)", async () => {
  ...
  store.syncFromTextExportRows([
    { memory_id: "m1", source_key: "k0", text: "first" },
  ]);
  const a = await store.resolve({ memory_id: "m1", source_key: "k0" } as { memory_id: string; source_key: string });
  expect(a.kind).toBe("string");
  if (a.kind === "string") {
    expect(a.string).toBe("first");
  }
  store.syncFromTextExportRows([{ memory_id: "m1", source_key: "k0", text: "second" }]);
  const b = await store.resolve({ memory_id: "m1", source_key: "k0" } as { memory_id: string; source_key: string });
...
test("SqliteLexicalStore two namespaces do not cross-pollute", async () => {
  ...
  const a = await s1.resolve(
    { memory_id: "m", source_key: "k" } as { memory_id: string; source_key: string },
  );
