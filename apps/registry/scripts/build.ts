...
const ENTRY = path.join(PKG_ROOT, "scripts", "start-registry.ts");
const OUT_DIR = path.join(PKG_ROOT, "dist", target);
const OUT_FILE = path.join(OUT_DIR, "khora-registry");

if (existsSync(OUT_DIR)) rmSync(OUT_DIR, { recursive: true, force: true });
mkdirSync(OUT_DIR, { recursive: true });

const defineFlags = [`--define=process.env.KHORA_PACKAGED=${JSON.stringify("1")}`];
