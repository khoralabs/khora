export function buildUserMessage(args: { threadText: string }): string {
  ...
  return [
    ...
    threadText,
    "",
    "Take your turn. Use OBP tools when you need to change the graph. If a deal is ready in this demo, the buyer should use the contextual obp_bind__* tool for the port they accept on the provider's offer. After binding (or if the negotiation is otherwise complete), call obp_end_negotiation instead of repeating confirmations—no further graph changes are needed.",
  ].join("\n");
}
