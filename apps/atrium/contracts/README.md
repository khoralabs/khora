...
This package depends only on `zod` and `@cfd/swarm-host` (for shared registration result types). Keep it dependency-light — adding a runtime dependency here forces it onto every downstream package.
