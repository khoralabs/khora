  ...
  const duplex: DuplexByteStream = {
    ...
    async write(bytes: Uint8Array) {
      ...
      try {
        ...
        console.error("[atrium-host] duplex unix socket write failed", err);
      }
    },
    async close() {
      if (closed) return;
  ...
  const listener = Bun.listen<DuplexUnixSocketData>({
    ...
    socket: {
      ...
      data(socket, chunk) {
        ...
        void settleDuplexUnixSession(socket, opts.deps, opts.snapshotLimit).catch((err) => {
      ...
      error(_socket, err) {
        console.error("[atrium-host] duplex unix socket error", err);
      },
    },
  });

  console.warn(`[atrium-host] Duplex unix ingress listening on ${opts.unixPath}`);
  return {
