...
export type AgentIdentityFile = {
  did: string;
  /** Multicodec/base64pad-encoded private key as produced by `EdDSASigner.export()`. */
  encoded: string;
};
...
export async function loadIdentity(filePath: string): Promise<EdDSASigner | undefined> {
  let text: string;
  try {
...
export async function saveIdentity(filePath: string, signer: EdDSASigner): Promise<void> {
...
export async function loadOrCreateIdentity(filePath: string): Promise<EdDSASigner> {
