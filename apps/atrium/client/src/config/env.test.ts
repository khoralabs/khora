...
import { atriumAppConfigFromEnv } from "./env.ts";

describe("atriumAppConfigFromEnv", () => {
  test("translates known env vars to AtriumAppConfigBase fields", () => {
    ...
    expect(atriumAppConfigFromEnv(env)).toEqual({
      ...
      plugins: {
        ...
        [ATRIUM_BUILTIN_PLUGIN_ID.telemetry]: { dir: "telemetry", maxFileBytes: 1048576 },
  ...
  test("empty / unset env yields empty partial", () => {
    expect(atriumAppConfigFromEnv({} as NodeJS.ProcessEnv)).toEqual({});
  });
  ...
  test("telemetry max bytes defaults to 4MiB when unset", () => {
    const out = atriumAppConfigFromEnv({
