/**
 * When EXEDRA_LITESTREAM is set, runs Litestream (exedra.db + watched memories/)
...
async function runWithLitestream(): Promise<void> {
  const s3 = readLitestreamS3Env("exedra/litestream");
  ...
  const yaml = buildLitestreamYaml({
    ...s3,
    dbs: [
      { kind: "file", path: exedraDbAbs, replicaSuffix: "exedra.sqlite" },
      {
        kind: "dir",
        dir: memoriesDirAbs,
        pattern: "*.db",
