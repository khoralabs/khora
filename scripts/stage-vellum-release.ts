...
export function cliMetaPkgJson({
  ...
  return {
    ...
    repository: { type: "git", url: repoUrl, directory: "apps/vellum/cli" },
...
export function daemonMetaPkgJson({
  ...
  return {
    ...
    repository: { type: "git", url: repoUrl, directory: "apps/vellum/daemon" },
...
export function platformPkgJson({
  ...
  return {
    ...
    license: "MIT",
    author: "Khora Labs",
    repository: { type: "git", url: repoUrl, directory: `apps/vellum/${kind}` },
    os: [target.os],
    cpu: [target.cpu],
    files: [binName],
  };
...
export async function stageVellumRelease(opts: StageOptions): Promise<StageResult> {
  ...
  const daemonReadme = path.join(workspaceRoot, "apps/vellum/daemon/README.md");
...
if (import.meta.main) {
  ...
  const releaseDir = path.join(workspaceRoot, "apps/vellum/release");
