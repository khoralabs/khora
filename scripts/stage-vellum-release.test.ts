...
describe("stageVellumRelease", () => {
  ...
  beforeEach(() => {
    ...
    releaseDir = path.join(workspace, "apps/vellum/release");

    mkdirSync(path.join(workspace, "apps/vellum/cli/scripts"), { recursive: true });
    mkdirSync(path.join(workspace, "apps/vellum/cli/assets/configs"), { recursive: true });
    mkdirSync(path.join(workspace, "packages/vellum/client"), { recursive: true });

    writeFileSync(
      path.join(workspace, "apps/vellum/cli/scripts/postinstall.ts"),
    ...
    for (const name of [
      ...
      writeFileSync(
        path.join(workspace, "apps/vellum/cli/assets/configs", name),
        `{ "name": "${name}" }`,
      );
    }
    writeFileSync(
      path.join(workspace, "packages/vellum/client/vellum-config.schema.json"),
      '{"$id":"vellum"}',
    );
  });
