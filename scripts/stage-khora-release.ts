 ...
 *   apps/khora/daemon/dist/<bun-target>/khora-daemon        (cross-compiled bin)
 *   apps/khora/cli/assets/configs/{base,cli,daemon}.config.json
 *   apps/khora/client/khora-config.schema.json              (built via build:schema)
...
export function cliMetaPkgJson({
  ...
  return {
    ...
    version,
    description:
      "CLI for the Khora agent host. Signs requests with a local Ed25519 identity, manages plugins, and supervises the inbox daemon. Native binaries; no runtime required.",
    license: "MIT",
    author: "Khora Labs",
...
export function daemonMetaPkgJson({
  ...
  return {
    ...
    keywords: ["khora", "agent", "daemon", "khoralabs"],
    type: "module",
...
export async function stageKhoraRelease(opts: StageOptions): Promise<StageResult> {
  ...
  const schemaSrc = path.join(workspaceRoot, "apps/khora/client/khora-config.schema.json");
