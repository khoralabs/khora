...
async function bumpCli(workspaceRoot: string, version: string): Promise<void> {
  ...
  if (!existsSync(manifestPath)) {
    console.error(`missing ${manifestPath}; run package-release-tarballs.ts cli first`);
    process.exit(1);
  }
...
async function bumpServer(workspaceRoot: string, version: string): Promise<void> {
  ...
  if (!existsSync(manifestPath)) {
    console.error(`missing ${manifestPath}; run package-release-tarballs.ts server first`);
    process.exit(1);
  }
