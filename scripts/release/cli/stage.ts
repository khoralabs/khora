...
import { type PlatformTarget, SUPPORTED_SLUGS, SUPPORTED_TARGETS } from "../targets";

export type { PlatformTarget };
export { SUPPORTED_SLUGS, SUPPORTED_TARGETS };
...
if (import.meta.main) {
  ...
  if (!version || !/^\d+\.\d+\.\d+(?:-[\w.-]+)?$/.test(version)) {
    console.error("usage: scripts/release/cli/stage.ts <semver> [--registry-url=<url>]");
    process.exit(1);
  }
