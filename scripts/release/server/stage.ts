...
import { ensureLitestreamForTarget } from "../../litestream/ensure-for-target";
import { type PlatformTarget, SUPPORTED_TARGETS } from "../targets";
  ...
  for (const target of SUPPORTED_TARGETS) {
    ...
    if (copyBinaries) {
      ...
      await ensureLitestreamForTarget(
        ...
        path.join(workspaceRoot, "apps/server/.bin/litestream"),
      );
...
if (import.meta.main) {
  ...
  if (!version || !/^\d+\.\d+\.\d+(?:-[\w.-]+)?$/.test(version)) {
    console.error("usage: scripts/release/server/stage.ts <semver>");
    process.exit(1);
  }
